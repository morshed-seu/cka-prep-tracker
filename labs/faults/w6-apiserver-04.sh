#!/usr/bin/env bash
# CKA fault drill w6-apiserver-04 (lesson 6.6) — plants ONE fault in the
# kube-apiserver static-pod manifest of the LAB cluster.
# Run on the control-plane VM:  sudo bash w6-apiserver-04.sh
...
echo "============================================================"
echo " CKA fault drill w6-apiserver-04"
...
if ! grep -Eq 'image: registry\.k8s\.io/kube-apiserver:v[0-9.]+$' "$MANIFEST"; then
...
# ---- plant ----
sed -i -E 's|(image: registry\.k8s\.io/kube-apiserver:v[0-9.]+)$|\1-broken|' "$MANIFEST"
