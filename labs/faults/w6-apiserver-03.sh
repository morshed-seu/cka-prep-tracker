#!/usr/bin/env bash
# CKA fault drill w6-apiserver-03 (lesson 6.6) — plants ONE fault in the
# kube-apiserver static-pod manifest of the LAB cluster.
# Run on the control-plane VM:  sudo bash w6-apiserver-03.sh
...
echo "============================================================"
echo " CKA fault drill w6-apiserver-03"
...
if ! grep -q -- '--tls-cert-file=/etc/kubernetes/pki/apiserver.crt' "$MANIFEST"; then
...
# ---- plant ----
sed -i 's|--tls-cert-file=/etc/kubernetes/pki/apiserver.crt|--tls-cert-file=/etc/kubernetes/pki/apiserver-tls.crt|' "$MANIFEST"
